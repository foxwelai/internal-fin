...
export function CategoryBreakdownChart({ data }: { data: CategorySlice[] }) {
  ...
  return (
    // Sized by its card, not the viewport: beside an open sidebar on a tablet
    // the card is too narrow for ring and list side by side.
            ...
            <PieChart>
              <Pie
                ...
                onMouseEnter={(_, index) =>
                  setActive(data[index]?.category ?? null)
                }
              ...
              >
                {data.map((slice) => (
                  <Cell
                    ...
                    fill={slice.color}
                    opacity={
                      active === null || active === slice.category ? 1 : 0.35
                    }
                  />
                ))}
              </Pie>
